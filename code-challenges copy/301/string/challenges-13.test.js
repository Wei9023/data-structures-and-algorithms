...
const firstLetters = (arr) => {
  let newArr = arr.map(value => value.charAt(0));
  return newArr;
};
...
const findHappiness = (arr) => {
  let sub = arr.filter(value => value.includes(':)'));
  return sub;
};
...
const standardizePhoneNumbers = (arr) => {
  let phoNum = arr.map(value => (value.substring(1,4)+value.substring(6,9)+value.substring(10,14)));
  return phoNum;
};
...
const onlyOddChars = (str) => {
    ...
    for(let i =0; i<str.length; i++ ){
         if(i%2!==0){
            oddLetter = str.charAt(i);
            letter.push(oddLetter);
         }
    }
    return letter.join('');
};
...
const unenrollBrook = (arr) => {
  let newArr = arr.map(value => value.filter(name => !name.includes('Brook')));
  return newArr;
};
