...
const notInFirstArray = (forbiddenValues, arr) => {
     let filteredNum = arr.filter((n) => !forbiddenValues.includes(n));
     return filteredNum;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let greater = arr.filter(obj => (obj.baseStat > minBaseStat));
  return greater;
};
...
const getStatName = (arr, minBaseStat) => {
    let greater = arr.filter(obj => (obj.baseStat > minBaseStat));
    let greaterName = greater.map(item => item.stat.name);
    return greaterName;
};
...
const getCharactersWithoutChildren = (arr) => {
    let noChildren = arr.filter(obj => !(obj.children));
    return noChildren;
};
...
const evenOddNumericValues = (arr) => {
  let num = arr.filter(n => (n !== undefined && typeof(n) ==='number') && !isNaN(n));
  let evenOdd = num.map(value =>{
      if(value % 2){
        return 'odd'
      } else if (!(value % 2)){  
        return 'even'
