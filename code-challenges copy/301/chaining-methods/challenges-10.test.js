...
const divisibleByFiveTwoToThePower = (input) => {
  let newArr = input.map(element =>{
    if( element === []){
        ...
        return element.filter( value => value % 5 === 0 && typeof(value)==='number')
    }
  });
  let result = newArr.map(element => {
    if( element === []){
        ...
        return element.map(item => Math.pow(2, item))
    }
  });
  return result;
};
...
let findMaleAndFemale = (data) => {
  let nameArr = data.filter((value) => value.gender==='male' || value.gender ==='female');
  return nameArr.map(element => element.name).join(' and ');
}
...
let findShortest = (data) => {
  let arr = data.map(element => element.name);
  let shortestWord = arr.reduce(function(prev, next) {
