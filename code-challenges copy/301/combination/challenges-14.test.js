...
const toTitleCase = (arr) => {
  let newArr = arr.map(value => value.slice(0,1).toUpperCase() + value.slice(1));
  return newArr;
};
...
let biggerThanLuke = (arr) => {
    let nameArr = arr.filter(obj => parseInt(obj.mass) > 77);
    let name = nameArr.map(value => value.name);
    let str = name.join(' - ');
    return str;
}
