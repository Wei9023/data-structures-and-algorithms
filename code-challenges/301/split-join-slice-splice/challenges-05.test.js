...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(element => {
    result.push(element.slice(element.indexOf(' ', element.indexOf(' ', 0)+1)+1));
    return result;
  });
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(element => {
    ...
    let tmpValue = "";
    for(let i = 2; i<splitArray.length; i++){
        tmpValue = tmpValue + splitArray[i];
...
const removeEvenValues = (arr) => {
  let tmpArr = []; // stores all the odd values, 
  arr.forEach(element => {
    if(element % 2 === 1 ) {
      tmpArr.push(element);
    }
  ...
  tmpArr.forEach(element => { // copy all the odd values into variable arr,
    arr.push(element)
  });
