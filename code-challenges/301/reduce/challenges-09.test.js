...
const reversedString = (str) => {
  return str.split('').reduce((rev, value) => value + rev,'');
};
...
const extractState = (statName, arr) => {
  let findName = arr.reduce(function(acc, obj){
    let key = obj.name;
        console.log(key)
        if(key === statName){
            return obj;
...
const extractChildren = (arr) => {
  let includeA = arr.filter((object) => (/a/g).test(object.name));
  let childrenArr =[];
  includeA.reduce(function(acc, obj){
    if(!obj.children){
        return null;
    } else {
        let nameArr = obj.children
