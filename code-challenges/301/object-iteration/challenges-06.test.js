...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(element => {
      if( element.name === character){
        let characterArr = Object.values(arr[arr.indexOf(element)]); 
        if (characterArr[2].length > 0){
            result = true;
        } else if(characterArr[2].length === 0){
            result = false;
        } 
      }
  })
...
const hasChildrenEntries = (arr, character) => {
    ...
    arr.forEach(element => {
        if( element.name === character){
          let characterArr = Object.entries(arr[arr.indexOf(element)]);
          console.log(characterArr);
          if ((characterArr[2])[1].length > 0){
              result = true;
          } else if((characterArr[2])[1].length === 0) {
              result = false;
          } 
        }
    })
